import type { ProxyConfig, ManagedProxy, ProxyStatus } from '../types/index.js'
...
type ProxyManager = {
  ...
  getStatus: () => { active: number; blocked: number; cooldown: number }
}

// Bright Data 세션 ID 기반 프록시 풀 생성
// 같은 호스트/포트에 다른 세션 ID를 붙여 별도 IP로 취급
const createProxyManager = (
  ...
  const findProxy = (config: ProxyConfig): ManagedProxy | undefined => {
  ...
  return {
    ...
    reportFailure: (config: ProxyConfig): void => {
