...
// 검색 URL 생성
const buildSearchUrl = (marketplace, keyword, pageNumber) => {
...
// CAPTCHA/차단 감지
const detectBlock = async (page) => {
    const captcha = await page.$(SEARCH_SELECTORS.captcha);
    if (captcha)
        return true;
    // HTTP 503 Service Unavailable 체크
    const title = await page.title();
    if (title.includes('Sorry') || title.includes('Robot Check'))
        return true;
    return false;
};
// 검색 결과 페이지 파싱
const scrapeSearchPage = async (page, marketplace, keyword, pageNumber) => {
