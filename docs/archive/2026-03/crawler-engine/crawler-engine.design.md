...
> **Author**: Claude (AI)
> **Date**: 2026-03-02
> **Status**: Implemented (코드 완성, 배포/테스트 대기)
...
// 3. Proxy Manager 생성 (Bright Data 풀)
// 4. Google Chat Notifier 생성 (선택적)
// 5. BullMQ Queue + Worker 생성
// 6. Scheduler 시작
// 7. Graceful Shutdown 핸들링 (SIGTERM, SIGINT)
//    - Worker 중지 (진행 중 잡 완료 대기)
//    - Queue 연결 종료
//    - 브라우저 인스턴스 정리
//    - Redis 연결 종료
//    - Google Chat 종료 알림 전송
```

### 5.15 Logger (`logger.ts`)

```typescript
// 구조화 로그 (JSON stdout/stderr, Railway/Docker 로그 수집 호환)
const log = (
  level: 'info' | 'warn' | 'error',
  module: string,
  message: string,
  extra?: { campaignId?: string; asin?: string; error?: string; duration?: number },
): void => {
  // LogEntry 형태로 JSON 직렬화
  // error 레벨 → stderr, 나머지 → stdout
}
```

### 5.16 Google Chat Notifications (`google-chat.ts`)

크롤링 결과를 Spigen 팀 Google Chat 채널에 실시간 알림합니다.
...
// Google Chat Webhook 기반 알림
// - webhookUrl이 null이면 모든 호출 무시 (선택적 기능)
// - 전송 실패 시 로그만 남기고 크롤링 자체에 영향 없음
const createChatNotifier = (webhookUrl: string | null): ChatNotifier => { ... }

// 알림 예시:
// ✅ [Sentinel Crawler] "spigen case" 캠페인 완료
// 수집: 45건 | 전송: 38건 | 중복: 5건 | 에러: 2건
// 소요: 3분 42초
//
// ❌ [Sentinel Crawler] "spigen case" 캠페인 실패
// 원인: All proxies exhausted after 3 retries
```

**환경 변수**: `GOOGLE_CHAT_WEBHOOK_URL` (선택적)

### 5.17 Follow-up Module (`follow-up/`)

신고 후 리스팅 삭제/수정 여부를 크롤러로 재방문하여 감지합니다.
...
// Web API 연동
// GET /api/monitoring/pending → PendingResponse (재방문 대상 목록)
// POST /api/monitoring/callback → CallbackResponse (스냅샷 저장 + AI 판단)
```
...
### Phase 6: BullMQ 스케줄러 + 부가 모듈 (7 items)
...
| 22 | `crawler/src/logger.ts` | 구조화 JSON 로거 | ✅ |
| 23 | `crawler/src/notifications/google-chat.ts` | Google Chat 알림 | ✅ |
| 24 | `crawler/src/follow-up/types.ts` | 팔로업 타입 정의 | ✅ |
| 25 | `crawler/src/index.ts` | 메인 엔트리 | ✅ |

### Phase 7: pnpm workspace 통합 (1 item)

| # | 파일 | 설명 | Status |
|---|------|------|:------:|
| 26 | `pnpm-workspace.yaml` | workspace에 crawler 추가 | ✅ |

**총 26개 구현 항목** (Crawler 21개 + Web API 5개) — **전 항목 완료**
...
│  │ (Sentinel Web) │◄──│ (Crawler + SC Bot) │──▶│ (DB/Storage) │  │
│  │ Next.js        │   │ Node.js Container  │   │ PostgreSQL   │  │
│  └───────────────┘   └───────────────────┘   └──────────────┘  │
│                              │                                   │
│                       ┌──────▼──────┐                            │
│                       │ Upstash     │                            │
...
| 플랫폼 | Railway.com |
| 예상 비용 | $5~20/월 |
| 컨테이너 | Node.js + Playwright (Chromium) |
| 스케일 | 단일 인스턴스 (1 worker) |
...
| 변수 | 필수 | 설명 |
|------|:----:|------|
| `SENTINEL_API_URL` | O | Web API URL (Vercel) |
| `SENTINEL_SERVICE_TOKEN` | O | Crawler 인증 토큰 |
| `UPSTASH_REDIS_URL` | O | BullMQ 큐 저장소 |
| `UPSTASH_REDIS_TOKEN` | O | Redis 인증 |
| `BRIGHTDATA_PROXY_HOST` | O | 프록시 호스트 |
| `BRIGHTDATA_PROXY_PORT` | O | 프록시 포트 |
| `BRIGHTDATA_PROXY_USERNAME` | O | 프록시 사용자 |
| `BRIGHTDATA_PROXY_PASSWORD` | O | 프록시 비밀번호 |
| `GOOGLE_CHAT_WEBHOOK_URL` | - | 알림 Webhook (선택) |
| `PROXY_POOL_SIZE` | - | 프록시 풀 크기 (기본: 5) |
| `CONCURRENCY` | - | 동시 크롤링 수 (기본: 3) |

---

## 12. SC 자동화 아키텍처 (향후 확장)

### 12.1 현재 구현: Semi-Auto

```
오퍼레이터 PC
  ├─ Chrome Extension → SC 폼 자동 채우기
  └─ 오퍼레이터가 직접 확인 후 Submit 클릭
```

### 12.2 향후 확장: Full-Auto (Cookie Injection 방식)

사용자 논의 결과, **Cookie Injection** 방식을 채택:

```
1. 오퍼레이터가 아침에 1회 SC 로그인 (Chrome Extension에서)
2. Extension이 SC 쿠키를 서버로 전송
3. 서버 Playwright가 주입된 쿠키로 SC 자동 신고
4. 쿠키 만료 시 Extension이 재전송 알림
```

**아키텍처 다이어그램:**

```
┌──── 오퍼레이터 PC ────┐       ┌──── Railway Server ────┐
│                        │       │                         │
│  Chrome Extension      │       │  SC Bot (Playwright)    │
│  ┌──────────────────┐  │       │  ┌───────────────────┐  │
│  │ SC 로그인 감지    │──┼──────▶│  │ 쿠키 저장 (암호화) │  │
│  │ 쿠키 추출/전송    │  │  API  │  │ SC 폼 자동 채우기  │  │
│  │ 만료 알림         │  │       │  │ 신고 제출 자동화   │  │
│  └──────────────────┘  │       │  └───────────────────┘  │
└────────────────────────┘       └─────────────────────────┘
```

**장점**: OTP/비밀번호 저장 불필요, 보안 리스크 최소화
**제약**: 쿠키 유효시간 (보통 8~12시간), 하루 1회 수동 로그인 필요

### 12.3 SC 자동화 구현 우선순위

| 단계 | 범위 | 상태 |
|------|------|------|
| 1 | Semi-Auto (Extension 폼 채우기) | ✅ 완료 |
| 2 | Cookie Injection API (Extension → Server) | 미구현 |
| 3 | Server SC Bot (Playwright + 쿠키) | 미구현 |
| 4 | 쿠키 만료 감지 + 재전송 알림 | 미구현 |
...
| 0.2 | 2026-03-02 | 구현 완료 반영 — Logger, Google Chat, Follow-up 모듈 추가, 배포 전략 (Railway), SC 자동화 아키텍처, 전 항목 ✅ (26 items) | Claude (AI) |
