...
개발자 C (백엔드): 공통 인프라 + Finance + Logistics
  담당: src/app/(protected)/finance/*, modules/finance/*
         src/app/(protected)/logistics/*, modules/logistics/*
...
| Vercel 빌드 시간 증가 | Low | Medium | 모듈별 dynamic import, 필요시 Turborepo 전환 |
...
- [ ] 모듈 스위처가 사이드바에 표시됨
- [ ] 기존 IP 기능이 `/ip/*` URL에서 정상 동작
- [ ] 기존 URL(`/campaigns` 등)이 `/ip/campaigns`으로 리다이렉트
- [ ] Extension이 새 URL에서도 정상 동작
- [ ] `modules/` 폴더 구조가 생성되어 있음
- [ ] `ads` DB 스키마가 존재함
...
> **지금 Sentinel을 "백화점의 1층"으로 만들고, 나머지 6개 모듈을 2~7층으로 추가하는 구조.**
...
| 7층 | **Logistics** | 재고 관리, FBA 출고, 배송 추적 | 예정 |
