...
### 요약
Brand Registry 자동 신고 후 케이스 관리 및 안정화 단계 완성. 4개 트랙 16개 항목: Crawler Worker 모니터링, Web BR 통합(템플릿 임포트, 재신고), 케이스 관리 UI(Smart Queue, SLA 뱃지, AI 분류), PD 팔로업 자동화. 설계-구현 일치율 100% (v1.0 84% → v2.0 100%, 1회 반복).

### Added
- **Crawler Infrastructure**
  - `crawler/src/heartbeat.ts`: 30초 주기 워커 모니터링, 3회 연속 실패 감지, Google Chat 알림
  - `src/lib/heartbeat.ts` (Web): Worker 상태 추적, 복구 감지
  - Daily Report (GChat): 일일 처리 건수, 큐 잔량, Uptime 요약 (KST 09:00)

- **BR Template Management**
  - `src/lib/br-template/parse-excel.ts`: Excel/CSV 파싱 (case-insensitive, 다중 구분자)
  - `src/app/api/br-templates/upload/route.ts`: 템플릿 대량 업로드 (5MB 제한)
  - `BrTemplateSettings.tsx`: 템플릿 관리 UI (활성화, 삭제, 통계)
  - `br-template-examples.ts`: AI 드래프트용 Few-Shot 통합

- **Case Management UI**
  - `BrCaseQueueBar.tsx`: Smart Queue 4분류 (action_required, sla_warning, new_reply, stale)
  - `SlaBadge.tsx`: SLA 카운트다운 (Green/Yellow/Red/Gray), 60초 갱신
  - `CaseChain.tsx`: 케이스 재신고 이력 시각화 (parent_report_id 네비게이션)
  - Bulk Actions: 일괄 재신고, 일괄 아카이브 (audit logging)

- **PD Follow-up**
  - `crawler/src/pd-followup/`: 자동 팔로업 스케줄러 (daily 12:00 PST, 7일 간격)
  - `pd-followup-pending/result` API: 리스팅 변화 감지 (제목/가격/판매자/이미지/삭제)
  - `SnapshotViewer.tsx`: 스냅샷 비교 UI, 변화 diff, 타임라인 네비게이션
  - Per-report override: 개별 리포트 팔로업 주기 설정

- **AI Enhancements**
  - Form-config 필드 주입: AI 드래프트가 폼 타입별 정확한 필드 생성 (`getBrFormContext()`)
  - Amazon 답변 분류: 5-class 자동 태깅 (approved, rejected, info_needed, in_progress, partial)

### Changed
- `br_submit_data`: `subject` 제거, `asins`/`orderId` 추가
- Extension: BR MAIN world 시도 코드 제거, PD reporting 전용화
- SC → PD 전체 파일명 변경 (60+ 파일): 크롤러, 익스텐션, 웹
- `crawler/src/br-submit/form-config.ts`: SSoT 중앙 집중식 폼 정의

### Fixed
- A1: Heartbeat 모니터링 로직 완성 (v1.0 PARTIAL → DONE)
- B1: AI 드래프트 form-config 필드 주입 (v1.0 PARTIAL → DONE)
- B3: Excel 템플릿 전체 파이프라인 (v1.0 NOT_IMPL → DONE)

### Metrics
- **Design Match Rate v1.0**: 84% (12 DONE, 3 PARTIAL, 1 NOT_IMPL)
- **Design Match Rate v2.0**: 100% (16/16 DONE)
- **Iteration Count**: 1 (최소)
- **New Components**: 5+ UI 컴포넌트
- **New API Endpoints**: 5개
- **Files Modified**: 60+
- **Large Refactoring**: SC → PD 이름 변경

### Pre-Deployment
- DB: `rename-sc-to-pd.sql` Supabase 실행 필수 (코드 배포 전)
- Railway: heartbeat.ts, pd-followup, br-submit 변경사항 배포
- Vercel: Preview 검증 → Production 배포
...
## [2026-02-15] - PD Semi-Auto Submit via Extension
...
| 0.2 | 2026-02-15 | PD Semi-Auto Submit | ✅ Live |
