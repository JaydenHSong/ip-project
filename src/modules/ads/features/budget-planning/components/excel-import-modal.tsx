        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <button
              ...
              className="w-full rounded-lg border-2 border-dashed border-gray-300 px-4 py-6 text-center hover:border-orange-400 transition-colors"
            >
              {selectedFile ? (
                <div>
                  <p className="text-sm font-medium text-gray-900">{selectedFile.name}</p>
                  <p className="mt-1 text-xs text-gray-400">{(selectedFile.size / 1024).toFixed(1)} KB</p>
                </div>
              ) : (
                <div>
                  <p className="text-sm text-gray-500">Click to select CSV or Excel file</p>
                  <p className="mt-1 text-xs text-gray-400">Supported: .csv, .xlsx, .xls</p>
                </div>
              )}
            </button>
          </div>
