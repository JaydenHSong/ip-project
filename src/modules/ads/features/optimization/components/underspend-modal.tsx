...
const UnderspendModal = ({ campaignId, campaignName, dailyBudget, spendToday, isOpen, onClose }: UnderspendModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-[520px] rounded-xl bg-surface-card shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-th-border px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-th-text">Underspend Analysis</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            <p className="text-xs text-th-text-muted">{campaignName}</p>
            <p className="text-[10px] text-th-text-muted mt-0.5">Budget utilization: {utilization.toFixed(0)}%</p>
          </div>

          {/* Hero Number + Progress Bar — Design M05 */}
          <div className="rounded-lg border border-th-border bg-th-bg-hover p-4 text-center">
            <p className="text-3xl font-bold text-th-text">${remaining.toFixed(2)}</p>
            <p className="text-xs text-th-text-muted mt-1">remaining of ${dailyBudget.toFixed(0)} daily budget</p>
            <ProgressBar value={utilization} size="md" className="mt-3" />
          </div>

          {/* Root Causes — Design M05: "contribution %, 설명, CTA" */}
          <div>
            <h3 className="text-sm font-medium text-th-text mb-3">Root Causes</h3>
            {isLoading ? (
              ...
              <div className="space-y-2">
                {causes.map((cause) => {
                  ...
                  return (
                    <div key={cause.id} className={`rounded-lg border p-3 ${isApplied ? 'border-emerald-200 bg-emerald-50' : 'border-th-border bg-surface-card'}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-semibold text-th-text">{cause.cause}</span>
                            <span className="rounded bg-orange-100 px-1.5 py-0.5 text-[10px] font-bold text-orange-700">
                              {cause.contribution_pct}%
                            </span>
                          </div>
                          <p className="text-xs text-th-text-muted">{cause.description}</p>
                          {/* Contribution bar */}
                          <div className="mt-2 h-1 w-full rounded-full bg-th-bg-tertiary">
                            <div
                        ...
                        <div className="shrink-0">
                          {isApplied ? (
                            ...
                            <button
                              ...
                              className="rounded bg-th-text px-3 py-1.5 text-[11px] font-medium text-white hover:bg-th-text"
                            >
                              {cause.fix_label}
                            </button>
                          )}
