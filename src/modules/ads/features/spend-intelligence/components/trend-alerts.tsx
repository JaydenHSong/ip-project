  ...
  if (items.length === 0) {
    return (
      <div className={`rounded-lg border border-th-border bg-surface-card p-4 ${className}`}>
        <h3 className="text-sm font-medium text-th-text mb-3">Trend Alerts</h3>
        <p className="text-sm text-th-text-muted text-center py-6">No concerning trends detected</p>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-lg border border-th-border bg-surface-card p-4 ${className}`}>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-medium text-th-text">Trend Alerts</h3>
        <span className="text-xs text-th-text-muted">{items.length} alerts</span>
      </div>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="mt-1 flex items-center gap-2 text-[11px]">
              ...
              {item.campaign_name && (
                <span className="text-th-text-muted">&middot; {item.campaign_name}</span>
              )}
            </div>
          </div>
        ))}
