...
export default async function ArchivedReportsPage() {
  ...
  if (isDemoMode()) {
    ...
    const { data, error } = await supabase
      .from('reports')
      .select('id, violation_type, violation_category, status, created_at, archived_at, archive_reason, listing_snapshot, listings!reports_listing_id_fkey(asin, title, marketplace, seller_name)')
      .eq('status', 'archived')
