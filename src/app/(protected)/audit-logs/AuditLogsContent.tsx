...
export const AuditLogsContent = ({ logs, totalPages, page, actionFilter }: AuditLogsContentProps) => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-th-text">{t('auditLogs.title')}</h1>

      <div className="flex gap-2">
        <Link
          href="/audit-logs"
          className={`rounded-lg px-3 py-1.5 text-sm ${!actionFilter ? 'bg-th-accent-soft text-th-accent-text' : 'text-th-text-tertiary hover:bg-th-bg-hover'}`}
        >
          {t('common.all')}
        </Link>
        {['create', 'update', 'delete', 'approve', 'reject'].map((a) => (
          <Link
            ...
            className={`rounded-lg px-3 py-1.5 text-sm capitalize ${actionFilter === a ? 'bg-th-accent-soft text-th-accent-text' : 'text-th-text-tertiary hover:bg-th-bg-hover'}`}
          >
            {a}
          </Link>
        ))}
